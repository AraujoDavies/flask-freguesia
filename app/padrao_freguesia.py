...
def atualizar_historico_da_tbl_dp():
    ...
    stmt = select(tbl_dp).where(
        and_(tbl_dp.c.placar == None, tbl_dp.c.data_partida <= ontem)
    )
    updates = []
...
if __name__ == "__main__":
    ...
    for i in range(-1, 2, 1):  # 1 = ontem | 0 = hoje | -1 = amanha
        ...
        for match in main_matchs:
            ...
            if match_url in urls_analisada:
                continue
